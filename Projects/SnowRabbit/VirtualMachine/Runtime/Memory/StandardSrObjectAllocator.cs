...
{
    /// <summary>
    /// SnowRabbit 仮想マシンが使用する参照型メモリアロケータクラスです
    /// </summary>
    public class StandardSrObjectAllocator : MemoryAllocator<SrObject>
    {
        // 定数定義
        public const int MemoryBlockSize = 16;
        ...
        public const int RequireMinimumPoolSize = RequireMinimumPoolCount * MemoryBlockSize;
        private const int IndexNotFound = -1;

        // メンバ変数定義
        private SrObject[] memoryPool;
        ...
        /// <summary>
        /// メモリプールの容量を指定して StandardSrObjectAllocator のインスタンスを初期化します
        /// </summary>
        /// <param name="memoryPoolSize">確保しておくメモリプールのバイトサイズ。ただし、内部で SrObject 型のサイズが確保出来るように倍数調整が行われます。</param>
        /// <exception cref="ArgumentOutOfRangeException">poolSize が RequireMinimumPoolSize 未満です</exception>
        public StandardSrObjectAllocator(int memoryPoolSize)
        {
            // 事前の例外ハンドリングをして、要求サイズから確保するサイズを求めて配列を生成後初期化する
            ThrowExceptionIfInvalidPoolSize(memoryPoolSize);
            Initialize(new SrObject[ByteSizeToElementCount(memoryPoolSize, MemoryBlockSize)]);
        }


        /// <summary>
        /// メモリプールを直接渡して StandardSrObjectAllocator のインスタンスを初期化します
        /// </summary>
        ...
        public StandardSrObjectAllocator(SrObject[] memoryPool)
        {
        ...
        /// <exception cref="ArgumentNullException">memoryPool が null です</exception>
        /// <exception cref="ArgumentOutOfRangeException">memoryPool の長さが RequireMinimumPoolCount 未満です</exception>
        private unsafe void Initialize(SrObject[] memoryPool)
        {
        ...
        /// <summary>
        /// 要求されたオブジェクト確保数を満たすメモリの確保をします。実際の確保サイズは必ず一致しているかどうかを保証していません。
        /// </summary>
        ...
        public override MemoryBlock<SrObject> Allocate(int count, AllocationType type)
        {
            // 事前例外処理をして必要な空き領域を検索するが、見つけられなかったら
            ThrowExceptionIfRequestInvalidSizeOrType(count, type);
            var freeIndex = FindFirstFreeBlock(count);
            ...
            freePoolHeadIndex = DivideFreeMemoryBlock(freeIndex, count, type);


            // 見つけたインデックス+先頭管理領域分の位置とサイズでメモリブロックを生成して返す
            return new MemoryBlock<SrObject>(memoryPool, freeIndex + HeadMemoryInfoCount, count);
        }
        ...
        public override void Deallocate(MemoryBlock<SrObject> memoryBlock)
        {
        ...
        /// <exception cref="ArgumentNullException">memoryPool が null です</exception>
        /// <exception cref="ArgumentOutOfRangeException">memoryPool の長さが RequireMinimumPoolCount 未満です</exception>
        protected void ThrowExceptionIfInvalidMemoryPool(SrObject[] memoryPool)
        {
            // null の場合は
            if (memoryPool == null)
            {
