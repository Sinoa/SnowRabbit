...
- Reserved
  - 未定義の予約領域です。必ず 0 クリアされていなければなりません。
- R1
  - オペランド引数1のレジスタインデックスです。
- R2
  - オペランド引数2のレジスタインデックスです。
- R3
  - オペランド引数3のレジスタインデックスです。

レジスタインデックスは SnowRabbit.RuntimeEngine.VirtualMachine.SrvmProcessor クラスに
Register{RegisterName}Index として定数定義されています。
