...
namespace SnowRabbit.Compiler.Parser.SyntaxNodes
{
    ...
    public class RelationalExpressionSyntaxNode : SyntaxNode
    {
        ...
        public static SyntaxNode Create(LocalCompileContext context)
        {
            ...
            var expression = ShiftExpressionSyntaxNode.Create(context);
            while (token.Kind == TokenKind.OpenAngle || token.Kind == TokenKind.CloseAngle || token.Kind == TokenKind.LesserEqual || token.Kind == TokenKind.GreaterEqual)
            {
                // 実行するべきオペレーションを覚える
                var operation = token;
                ...
                var rightExpression = ShiftExpressionSyntaxNode.Create(context);


                // 自身の構文を生成する
                var relationalExpression = new RelationalExpressionSyntaxNode();
                relationalExpression.Add(expression);
                relationalExpression.Add(rightExpression);
