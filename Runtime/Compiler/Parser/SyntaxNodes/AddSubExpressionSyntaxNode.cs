...
namespace SnowRabbit.Compiler.Parser.SyntaxNodes
{
    ...
    public class AddSubExpressionSyntaxNode : SyntaxNode
    {
        ...
        public static SyntaxNode Create(LocalCompileContext context)
        {
            ...
            var expression = MulDivExpressionSyntaxNode.Create(context);
            while (token.Kind == TokenKind.Plus || token.Kind == TokenKind.Minus)
            {
                ...
                var rightExpression = MulDivExpressionSyntaxNode.Create(context);


                // 自身の構文を生成する
                var addSubExpression = new AddSubExpressionSyntaxNode();
                addSubExpression.Add(expression);
                addSubExpression.Add(rightExpression);
                addSubExpression.operation = operation;
