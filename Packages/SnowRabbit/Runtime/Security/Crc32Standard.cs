...
namespace SnowRabbit.Security
{
    ...
    public class Crc32Standard : Crc32
    {
        ...
        public override unsafe uint Calculate(byte[] buffer)
        {
            // バッファのアドレスを取得
            fixed (byte* p = buffer)
            {
        ...
        public override unsafe uint Calculate(byte[] buffer, int index, int count)
        {
            // もし buffer が null なら
            if (buffer == null)
            {
        ...
        /// <returns>計算された結果を返します</returns>
        /// <exception cref="ArgumentNullException">buffer が null です</exception>
        public override unsafe uint Calculate(byte* buffer, int count)
        {
            // uint.MaxValueによるXOR反転を利用した計算を行い結果を返す
