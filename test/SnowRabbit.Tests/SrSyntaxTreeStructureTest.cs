...
";
        ...
        Assert.That(compileUnit.Children.Count, Is.EqualTo(1));

        // 最初の子は関数宣言
        SyntaxNode funcNode = compileUnit.Children[0];
        Assert.That(funcNode, Is.InstanceOf<FunctionDeclareSyntaxNode>());

        // Children[0]: 戻り型 (int)
        Assert.That(funcNode.Children[0], Is.InstanceOf<TypeSyntaxNode>());
        Assert.That(funcNode.Children[0].Token.Kind, Is.EqualTo(SrTokenKind.TypeInt));

        // Children[1]: 関数名 (Add)
        Assert.That(funcNode.Children[1], Is.InstanceOf<IdentifierSyntaxNode>());
        Assert.That(funcNode.Children[1].Token.Text, Is.EqualTo("Add"));

        // Children[2]: パラメータリスト
        Assert.That(funcNode.Children[2], Is.InstanceOf<ParameterListSyntaxNode>());
        ParameterListSyntaxNode paramList = (ParameterListSyntaxNode)funcNode.Children[2];
        Assert.That(paramList.Children.Count, Is.EqualTo(2));

        // パラメータ1: int a
        ParameterSyntaxNode param1 = (ParameterSyntaxNode)paramList.Children[0];
        Assert.That(param1.Children[0].Token.Kind, Is.EqualTo(SrTokenKind.TypeInt));
        Assert.That(param1.Children[1].Token.Text, Is.EqualTo("a"));

        // パラメータ2: int b
        ParameterSyntaxNode param2 = (ParameterSyntaxNode)paramList.Children[1];
        Assert.That(param2.Children[0].Token.Kind, Is.EqualTo(SrTokenKind.TypeInt));
        Assert.That(param2.Children[1].Token.Text, Is.EqualTo("b"));

        // Children[3]: return文
        Assert.That(funcNode.Children[3], Is.InstanceOf<ReturnStatementSyntaxNode>());
    }
...
";
        ...
        CompileUnitSyntaxNode compileUnit = (CompileUnitSyntaxNode)node;
        Assert.That(compileUnit.Children.Count, Is.EqualTo(3));

        // 1つ目: global int counter = 100;
        GlobalVariableDeclareSyntaxNode var1 = (GlobalVariableDeclareSyntaxNode)compileUnit.Children[0];
        Assert.That(var1.Children[0].Token.Kind, Is.EqualTo(SrTokenKind.TypeInt));
        Assert.That(var1.Children[1].Token.Text, Is.EqualTo("counter"));
        Assert.That(var1.Children.Count, Is.EqualTo(3)); // 型、名前、初期値
        Assert.That(var1.Children[2], Is.InstanceOf<LiteralSyntaxNode>());
        Assert.That(var1.Children[2].Token.Integer, Is.EqualTo(100));
        ...
        Assert.That(var2.Children[0].Token.Kind, Is.EqualTo(SrTokenKind.TypeString));
        Assert.That(var2.Children[1].Token.Text, Is.EqualTo("message"));
        Assert.That(var2.Children[2].Token.Text, Is.EqualTo("hello"));

        // 3つ目: global number value; (初期値なし)
        GlobalVariableDeclareSyntaxNode var3 = (GlobalVariableDeclareSyntaxNode)compileUnit.Children[2];
        Assert.That(var3.Children[0].Token.Kind, Is.EqualTo(SrTokenKind.TypeNumber));
        Assert.That(var3.Children[1].Token.Text, Is.EqualTo("value"));
        Assert.That(var3.Children.Count, Is.EqualTo(2)); // 型と名前のみ
    }
...
";
        ...
        Assert.That(localVar1.Children[0].Token.Kind, Is.EqualTo(SrTokenKind.TypeInt));
        Assert.That(localVar1.Children[1].Token.Text, Is.EqualTo("x"));
        Assert.That(localVar1.Children.Count, Is.EqualTo(3)); // 型、名前、初期化式
        ...
        Assert.That(localVar2.Children[0].Token.Kind, Is.EqualTo(SrTokenKind.TypeString));
        Assert.That(localVar2.Children[1].Token.Text, Is.EqualTo("s"));
        Assert.That(localVar2.Children.Count, Is.EqualTo(2)); // 型と名前のみ
    }
...
";
        ...
        // Children[0]: 条件式 (x > 0)
        Assert.That(ifNode.Children[0], Is.InstanceOf<ExpressionSyntaxNode>());

        // Children[1]: if本体 (x = x - 1;)
        Assert.That(ifNode.Children[1], Is.InstanceOf<ExpressionSyntaxNode>());
    }
...
";
        ...
        // Children[2]: else文
        Assert.That(ifNode.Children[2], Is.InstanceOf<ElseStatementSyntaxNode>());
        ElseStatementSyntaxNode elseNode = (ElseStatementSyntaxNode)ifNode.Children[2];

        // else本体
        Assert.That(elseNode.Children[0], Is.InstanceOf<ExpressionSyntaxNode>());
    }
...
";
        ...
        // Children[0]: 条件式 (i < 10)
        Assert.That(whileNode.Children[0], Is.InstanceOf<ExpressionSyntaxNode>());

        // Children[1]: ループ本体 (i = i + 1;)
        Assert.That(whileNode.Children[1], Is.InstanceOf<ExpressionSyntaxNode>());
    }
...
";
        ...
        // Children[0]: 初期化式 (i = 0)
        Assert.That(forNode.Children[0], Is.InstanceOf<ExpressionSyntaxNode>());

        // Children[1]: 条件式 (i < 10)
        Assert.That(forNode.Children[1], Is.InstanceOf<ExpressionSyntaxNode>());

        // Children[2]: 更新式 (i = i + 1)
        Assert.That(forNode.Children[2], Is.InstanceOf<ExpressionSyntaxNode>());

        // Children[3]: ループ本体 (sum = sum + i;)
        Assert.That(forNode.Children[3], Is.InstanceOf<ExpressionSyntaxNode>());
    }
...
";
        ...
        CompileUnitSyntaxNode compileUnit = (CompileUnitSyntaxNode)node;
        Assert.That(compileUnit.Children.Count, Is.EqualTo(2)); // Add関数とMain関数
        ...
        // Children[0]: 関数名
        Assert.That(funcCall.Children[0].Token.Text, Is.EqualTo("Add"));

        // Children[1]: 引数リスト
        Assert.That(funcCall.Children[1], Is.InstanceOf<ArgumentListSyntaxNode>());
        ArgumentListSyntaxNode argList = (ArgumentListSyntaxNode)funcCall.Children[1];
        Assert.That(argList.Children.Count, Is.EqualTo(2)); // 2つの引数
    }
...
";
        ...
        CompileUnitSyntaxNode compileUnit = (CompileUnitSyntaxNode)node;
        Assert.That(compileUnit.Children.Count, Is.EqualTo(4)); // 3つの定数 + Main関数

        // 1つ目: #const MAX_VALUE 100
        ConstantDefineDirectiveSyntaxNode const1 = (ConstantDefineDirectiveSyntaxNode)compileUnit.Children[0];
        Assert.That(const1.Children[0].Token.Text, Is.EqualTo("MAX_VALUE"));
        Assert.That(const1.Children[1].Token.Integer, Is.EqualTo(100));

        // 2つ目: #const PI 3.14
        ConstantDefineDirectiveSyntaxNode const2 = (ConstantDefineDirectiveSyntaxNode)compileUnit.Children[1];
        Assert.That(const2.Children[0].Token.Text, Is.EqualTo("PI"));
        Assert.That(const2.Children[1].Token.Number, Is.EqualTo(3.14).Within(0.001));

        // 3つ目: #const NAME "test"
        ConstantDefineDirectiveSyntaxNode const3 = (ConstantDefineDirectiveSyntaxNode)compileUnit.Children[2];
        Assert.That(const3.Children[0].Token.Text, Is.EqualTo("NAME"));
        Assert.That(const3.Children[1].Token.Text, Is.EqualTo("test"));
    }
...
";
        ...
        Assert.That(return1.Children.Count, Is.EqualTo(1)); // 戻り値あり
        ...
        Assert.That(return2.Children.Count, Is.EqualTo(0)); // 戻り値なし (void)
    }
...
";
        ...
        // break文 (ifの2番目の子)
        Assert.That(ifNode.Children[1], Is.InstanceOf<BreakStatementSyntaxNode>());
    }
...
";
        ...
        Assert.That(expr.Children.Count, Is.GreaterThanOrEqualTo(2));
    }
}
