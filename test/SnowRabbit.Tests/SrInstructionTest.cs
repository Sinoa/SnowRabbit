...
public class SrInstructionTest
{
    ...
    [TestCase(OpCode.Cpf, SrvmProcessor.RegisterR10Index, SrvmProcessor.RegisterR13Index, (byte)0, 0x123U, (0b_1111_0001_00000000_0_01010_01101_00000UL << 32) | 0x123UL)]
    public void BitPatternTest(OpCode opCode, byte r1, byte r2, byte r3, uint imm, ulong expected)
    {
        // 値をセットして想定したビットパターンかを確認
        SrInstruction instruction = new SrInstruction();
        instruction.Set(opCode, r1, r2, r3, imm);
        Assert.That(instruction.Raw, Is.EqualTo(expected));
    }
    ...
    [TestCase(OpCode.Cpf, SrvmProcessor.RegisterR10Index, SrvmProcessor.RegisterR13Index, (byte)0, 0x123U)]
    public void GetParameterTest(OpCode opCode, byte r1, byte r2, byte r3, uint imm)
    {
        // 値をセットしてGet関数やフィールドから同様の値として取り出せるか確認
        SrInstruction instruction = new SrInstruction();
        instruction.Set(opCode, r1, r2, r3, imm);
        instruction.GetRegisterNumber(out byte r1Out, out byte r2Out, out byte r3Out);
        Assert.That(instruction.OpCode, Is.EqualTo(opCode));
        Assert.That(r1Out, Is.EqualTo(r1));
        Assert.That(r2Out, Is.EqualTo(r2));
        Assert.That(r3Out, Is.EqualTo(r3));
        Assert.That(instruction.Uint, Is.EqualTo(imm));
    }
}
